...
RSpec.describe GramsController, type: :controller do
  let(:gram) { FactoryBot.create(:gram, message: gram_message) }
  let(:gram_message) { 'a' }
  ...
  describe "grams#destroy action" do
    ...
    it "should allow a user to destroy grams" do
      ...
      gram_to_destroy = Gram.find_by_id(gram.id)
  ...
  describe "grams#update action" do
    it "shouldn't let users who didn't create the gram update it" do
      sign_in user
      patch :update, params: { id: gram.id, gram: { message: 'heck yes' } }
      expect(response).to have_http_status(:forbidden)
    end
    ...
    context 'gram has a message' do
      let(:gram_message) { "Initial Value" }
    ...
    it "should have http 404 error if the gram cannot be found" do
      sign_in user
      patch :update, params: { id: "nothx", gram: { message: 'Changed' } }
      expect(response).to have_http_status(:not_found)
    end
